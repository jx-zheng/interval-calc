...
void msubtract(double a, double b, double& ma, double&mb, bool init, bool minit);
        ...
        if(input == "exit"){
            ...
            if(tokens[0] == "enter"){
                if(stod(tokens[1]) > stod(tokens[2])){
                    std::cout << "Error: invalid interval as " << tokens[1] << " > " << tokens[2];
                    std::cout << std::endl;
                }
                else{
                ...
                ms(imm_val_a, imm_val_b, mem_val_a, mem_val_b, imm_is_initialized, mem_is_initialized);
            }
            else if(tokens[0] == "mr"){
                mr(imm_val_a, imm_val_b, mem_val_a, mem_val_b, imm_is_initialized, mem_is_initialized);
            }
            else if(tokens[0] == "mc"){
                mc(imm_val_a, imm_val_b, imm_is_initialized, mem_is_initialized);
            }
            else if(tokens[0] == "m+"){
                mplus(imm_val_a, imm_val_b, mem_val_a, mem_val_b, imm_is_initialized, mem_is_initialized);
            }
            else if(tokens[0] == "m-"){
                msubtract(imm_val_a, imm_val_b, mem_val_a, mem_val_b, imm_is_initialized, mem_is_initialized);
            }
                ...
                scalar_subtract(stod(tokens[1]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "scalar_multiply"){
                scalar_multiply(stod(tokens[1]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "scalar_divide"){
                scalar_divide(stod(tokens[1]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "scalar_divided_by"){
                scalar_divided_by(stod(tokens[1]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "interval_add"){
                interval_add(stod(tokens[1]), stod(tokens[2]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "interval_subtract"){
                interval_subtract(stod(tokens[1]), stod(tokens[2]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "interval_multiply"){
                interval_multiply(stod(tokens[1]), stod(tokens[2]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "interval_divide"){
                interval_divide(stod(tokens[1]), stod(tokens[2]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "intersect"){
                intersect(stod(tokens[1]), stod(tokens[2]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "integers"){
                integers(imm_val_a, imm_val_b, imm_is_initialized);
            }
            else if(tokens[0] == "cartesian_integers"){
                cartesian_integers(stod(tokens[1]), stod(tokens[2]), imm_val_a, imm_val_b, imm_is_initialized);
            }
            else{
